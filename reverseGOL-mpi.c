...
int main(int argc, char *argv[]) {
    ...
    if (rank!=0){
        ...
        for (int rk=1; rk < size; rk++){
            int work_fitness;
            char *work_plate = (char*)malloc(plate_size * sizeof(char));
            MPI_Recv(&work_fitness, 1, MPI_INT, rk, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            MPI_Recv(work_plate, plate_size, MPI_CHAR, rk, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            if(work_fitness < pop_fitness[best]){
                memcpy(population[best], work_plate, plate_size);
                pop_fitness[best] = work_fitness;
            }
            free(work_plate);
        }
        printf("\nBest fitness across ranks: %d\n", pop_fitness[best]);
        print_plate(population[best], n);
    }
